...
"""
sequence:list[int] = list(map(int,input('Podaj ciąg liczb: ').split()))

help_list:list[int] = [0 in range(len(sequence)-1)]
def merge(t_list:list[int],left:int, middle:int, right:int,help_list:list[int])->None:
    i:int = left
    j:int = middle + 1
    k:int = left
